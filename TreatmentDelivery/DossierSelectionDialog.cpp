    ...
    repo_(repo)
{
    ...
    try {
        ...
        QMessageBox::warning(this, "MTD", "Echec de l'extraction des dossiers de la base de données");
    }
    ConnectSignals();
}
...
void DossierSelectionDialog::GetSelectedDossier() {
    ...
    if (item) {
        ...
        QMessageBox::warning(this, "MTP", QString("Dossier non trouvé"));
    }
}
...
void DossierSelectionDialog::FillRow(int row, int dossier) {
    ...
    ui_->tableWidget->setItem(row, static_cast<int>(COLUMNS::SEANCE), new QTableWidgetItem(QString::number(repo_->GetActiveSeanceNumber(dossier) + 1)  +                                                                                               " (" + QString::number(static_cast<int>(repo_->GetSeances(dossier).size())) + ")"));
}
